...
Note (Cursor): Reference this template with @ in Cursor and instruct the agent to instantiate a new PRP file. Example: “Use @PRPs/templates/prp_base_typescript.md to create PRPs/ts-feature.md and fill all sections using patterns from @components and @__tests__.”
...
```powershell
# Development server validation (run in a new terminal if needed)
Start-Process -FilePath "npm" -ArgumentList "run","dev" | Out-Null
Start-Sleep -Seconds 5  # Allow Next.js startup time

# Page load validation
$head = Invoke-WebRequest -Method Head -Uri "http://localhost:3000/{feature-page}" -ErrorAction SilentlyContinue
if ($head.StatusCode -ne 200) { Write-Host "Page did not return 200" }

# API endpoint validation
$resp = Invoke-RestMethod -Method Post -Uri "http://localhost:3000/api/{resource}" -ContentType "application/json" -Body '{"test":"data"}'
...
$html = Invoke-WebRequest -Uri "http://localhost:3000/{page}" -UseBasicParsing
if ($html.Content -notmatch "expected-content") { Write-Host "Expected content not found" }
