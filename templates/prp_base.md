...
Note (Cursor): Reference this template with @ in Cursor and instruct the agent to instantiate a new PRP file. Example: “Use @PRPs/templates/prp_base.md to create PRPs/my-feature.md and fill all sections using patterns from @src and @tests.”
...
Start-Process -FilePath "uv" -ArgumentList "run","python","main.py" | Out-Null
Start-Sleep -Seconds 3  # Allow startup time

# Health check validation
try { Invoke-RestMethod -Method Get -Uri "http://localhost:8000/health" | Out-Null } catch { Write-Host "Service health check failed" }

# Feature-specific endpoint testing
$resp = Invoke-RestMethod -Method Post -Uri "http://localhost:8000/{your_endpoint}" -ContentType "application/json" -Body '{"test":"data"}'
$resp | ConvertTo-Json -Depth 6

# MCP server validation (if MCP-based)
# Test MCP tool functionality
$payload = '{"method": "tools/call", "params": {"name": "{tool_name}", "arguments": {}}}'
$payload | uv run python -m src.main

# Database validation (if database integration)
# Verify database schema, connections, migrations
psql $env:DATABASE_URL -c "SELECT 1;" 2>$null; if ($LASTEXITCODE -ne 0) { Write-Host "Database connection failed" }
...
# Example using oha (if installed): oha -n 100 -c 10 http://localhost:8000/{endpoint}

# Security Scanning (if security requirements)
bandit -r src/

# Load Testing (if scalability requirements)
# Example using k6 (if installed): k6 run script.js
